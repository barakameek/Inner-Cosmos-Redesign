...
const EncounterManager = (() => {
    ...
    function revealHiddenAspectTrait() { if (!isActive || !currentAspect || currentPlayerRef.insight < 1 || _getUnrevealedTraitCount() === 0) { let cost = 1; if(currentPlayerRef.detachedObservationActive){ cost = Math.max(0, cost -1); currentPlayerRef.detachedObservationActive = false; if(UIManager) UIManager.addLogEntry("Detached Observation reduces Insight cost.", "player_action_good");} if (currentPlayerRef.insight < cost && UIManager) UIManager.addLogEntry("Not enough Insight.", "warning"); else if (_getUnrevealedTraitCount() === 0 && UIManager) UIManager.addLogEntry("No more hidden traits.", "system"); return; } let cost = 1; if(currentPlayerRef.detachedObservationActive){ cost = Math.max(0, cost -1); currentPlayerRef.detachedObservationActive = false; if(UIManager) UIManager.addLogEntry("Detached Observation reduces Insight cost.", "player_action_good");} currentPlayerRef.modifyInsight(-cost, "revealing a Trait"); const unrevealed = currentAspect.hiddenTraits.filter(ht => !currentAspect.revealedTraits.includes(ht.name)); if (unrevealed.length > 0) { const traitToReveal = unrevealed[0]; currentAspect.revealedTraits.push(traitToReveal.name); if(UIManager) UIManager.addLogEntry(`Revealed: ${traitToReveal.name} - ${traitToReveal.description}`, "discovery"); } _updateEncounterUIDisplay(); if(UIManager && UIManager.getDOMElement('revealTraitEncounterButton')) UIManager.getDOMElement('revealTraitEncounterButton').disabled = (currentPlayerRef.insight < 1 || _getUnrevealedTraitCount() === 0); }
    ...
    const conceptCardEffects = {
        ...
        playDetachedObservation: (cardDef) => { currentPlayerRef.detachedObservationActive = true; if(UIManager) UIManager.addLogEntry("Perspective shifts: your next query will be more focused.", "system_positive"); currentPlayerRef.drawCards(1); },
