...
class Player {
    constructor() {
        // From CONFIG.INITIAL_PSYCHONAUT_NAME and CONFIG.INITIAL_AMBITION_TEXT
        // but name is set to "The Unknowing" by Game._updateHeaderInfo(true) initially
        // and actual name/ambition set by card effects/choices.
        ...
        this.attunements = { ...PLAYER_INITIAL_STATS.attunements }; // Deep copy for objects
        ...
        this.tarnishedLocketHopeAppliedThisTurn = false; // For MEM_TARNISHED_LOCKET effect
        // Temporary flags for card effects like "Detached Observation"
    ...
    drawFromAwakeningDeck() {
        if (this.awakeningDeck.length > 0) {
            if (this.hand.length >= CONFIG.MAX_HAND_SIZE) {
                if(UIManager) UIManager.addLogEntry("Hand is full. Cannot draw awakening insight.", "system_warning");
                return null;
            }
            const cardId = this.awakeningDeck.pop();
            if (cardId) {
                this.hand.push(cardId);
                const cardDef = CONCEPT_CARD_DEFINITIONS[cardId]; // Assumes global
                if(UIManager) UIManager.addLogEntry(`A flicker of insight: Drew "${cardDef?.name || 'an unknown concept'}".`, "discovery");
                return cardId;
            }
        } else { 
            if(UIManager) UIManager.addLogEntry("No more awakening insights to draw.", "system");
        }
        return null;
    }
    ...
    drawCards(count) { // Normal card draw from main deck
        ...
        for (let i = 0; i < count; i++) {
            if (this.hand.length >= CONFIG.MAX_HAND_SIZE) { if(UIManager) UIManager.addLogEntry("Hand is full. Cannot draw more cards.", "system_warning"); break; }
            if (this.deck.length === 0) {
                if (this.discardPile.length === 0) { if(UIManager) UIManager.addLogEntry("No concepts left in deck or discard pile to draw.", "system"); break; }
                if(UIManager) UIManager.addLogEntry("Deck empty. Reshuffling discard pile into deck.", "system");
                this.discardPile.forEach(cardId => this.deck.push(cardId)); this.discardPile = []; this.shuffleArray(this.deck);
                if (this.deck.length === 0) { if(UIManager) UIManager.addLogEntry("No concepts left after reshuffle.", "system"); break; }
            }
            ...
            if (cardId) {
                ...
                if (cardDef) {
                    if(UIManager) UIManager.addLogEntry(`Drew: ${cardDef.name}.`, "system_subtle");
                    if (cardDef.type === "Trauma" && cardDef.onDrawFunctionName) {
                         if(UIManager) UIManager.addLogEntry(`TRAUMA DRAWN: ${cardDef.name}! ${cardDef.description}`, "trauma_major");
                         if (Game && typeof Game.handleTraumaOnDraw === 'function') {
                            Game.handleTraumaOnDraw(cardId, cardDef.onDrawFunctionName);
                         }
                    }
                } else { if(UIManager) UIManager.addLogEntry(`Drew an unknown concept (ID: ${cardId}).`, "error"); }
            }
        }
        return drawnCards;
    }

    playCardFromHand(cardId) { // Returns cardDef if successful, null otherwise
        const cardIndex = this.hand.indexOf(cardId);
        if (cardIndex > -1) {
            const cardDef = CONCEPT_CARD_DEFINITIONS[cardId];
            if (!cardDef) { 
                console.error("Played card ID has no definition in Player.playCardFromHand:", cardId); 
                if(UIManager) UIManager.addLogEntry(`Error: Unknown concept ID "${cardId}" in hand.`, "error"); 
                return null; 
            }
            // Focus check is now primarily handled by EncounterManager.playConceptCard or Game._handleEncounterCardPlay
            // before this method is called. This method just handles moving the card.

            const playedCardId = this.hand.splice(cardIndex, 1)[0];
            // Traumas also go to discard after being "played" (their main effect is often onDraw or passive)
    ...
    discardCardFromHandById(cardId) { const cardIndex = this.hand.indexOf(cardId); if (cardIndex > -1) { const discardedCardId = this.hand.splice(cardIndex, 1)[0]; this.discardPile.push(discardedCardId); if(UIManager) UIManager.addLogEntry(`Discarded ${CONCEPT_CARD_DEFINITIONS[discardedCardId]?.name || 'a card'}.`, "system_negative"); return CONCEPT_CARD_DEFINITIONS[discardedCardId]; } return null; }
    ...
    addConceptToDeck(cardId, shuffleIn = false) { this.deck.push(cardId); if (shuffleIn) this.shuffleArray(this.deck); if(UIManager) UIManager.addLogEntry(`"${CONCEPT_CARD_DEFINITIONS[cardId]?.name || 'A new concept'}" coalesces in your deck.`, "reward"); }
    addConceptToDiscard(cardId) { this.discardPile.push(cardId); if(UIManager) UIManager.addLogEntry(`"${CONCEPT_CARD_DEFINITIONS[cardId]?.name || 'A concept'}" manifests in your discard pile.`, "system"); }
    addConceptToHand(cardId) { if (this.hand.length < CONFIG.MAX_HAND_SIZE) { this.hand.push(cardId); if(UIManager) UIManager.addLogEntry(`"${CONCEPT_CARD_DEFINITIONS[cardId]?.name || 'A concept'}" appears in your hand.`, "system_positive"); } else { if(UIManager) UIManager.addLogEntry(`Hand full, "${CONCEPT_CARD_DEFINITIONS[cardId]?.name || 'a concept'}" added to discard instead.`, "system_warning"); this.addConceptToDiscard(cardId); } }
    addTraumaToDiscard(traumaCardId) { this.discardPile.push(traumaCardId); const traumaDef = CONCEPT_CARD_DEFINITIONS[traumaCardId]; if(UIManager) UIManager.addLogEntry(`A Trauma (${traumaDef?.name || 'Unknown Fear'}) has formed in your discard pile!`, "trauma_major"); }
    removeCardFromPsyche(cardId) { let found = false; let pileName = ""; if (this.hand.includes(cardId)) { this.hand.splice(this.hand.indexOf(cardId), 1); found = true; pileName = "hand"; } else if (this.discardPile.includes(cardId)) { this.discardPile.splice(this.discardPile.indexOf(cardId), 1); found = true; pileName = "discard pile"; } else if (this.deck.includes(cardId)) { this.deck.splice(this.deck.indexOf(cardId), 1); found = true; pileName = "deck"; } if (found) { if(UIManager) UIManager.addLogEntry(`The Concept "${CONCEPT_CARD_DEFINITIONS[cardId]?.name}" fades from your ${pileName}.`, "system"); } else { if(UIManager) UIManager.addLogEntry(`Could not find "${CONCEPT_CARD_DEFINITIONS[cardId]?.name}" to remove.`, "warning"); } return found; }
    ...
    prepareForEncounter() {
        ...
        this.hand = [];
        if (!this.activePersonaStance && typeof PERSONA_STANCE_DEFINITIONS !== 'undefined' && PERSONA_STANCE_DEFINITIONS["OBSERVER"]) { this.setPersonaStance("OBSERVER"); }
    }
    ...
    startTurnInEncounter() { 
        ...
        this.detachedObservationActive = false; // Reset flag at start of turn
    }
}
