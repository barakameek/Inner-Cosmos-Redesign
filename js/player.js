...
class Player {
    constructor() {
        ...
        // Flag for Tarnished Locket effect, to ensure it only triggers once per "day"/turn if hope stays low
    ...
    drawFromAwakeningDeck() {
        if (this.awakeningDeck.length > 0) {
            ...
            if (cardId) {
                ...
                // On-draw effects for awakening cards are handled when they are played or by their type
                return cardId;
            }
        } else {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (this.deck.length === 0) {
                if (this.discardPile.length === 0) {
                    UIManager.addLogEntry("No concepts left in deck or discard pile to draw.", "system");
                    break;
                }
                UIManager.addLogEntry("Deck empty. Reshuffling discard pile into deck.", "system");
                this.discardPile.forEach(cardId => this.deck.push(cardId));
                this.discardPile = [];
                this.shuffleArray(this.deck);
                if (this.deck.length === 0) {
                    UIManager.addLogEntry("No concepts left after reshuffle.", "system");
                    break;
                }
            }
            ...
            if (cardId) {
                ...
                if (cardDef) {
                    ...
                    if (cardDef.type === "Trauma" && cardDef.onDrawFunctionName) {
                        // Game.executeOnDrawEffect(cardId, cardDef.onDrawFunctionName); // Main loop handles this
                        // For now, just log. Main.js will need to explicitly call this.
                         UIManager.addLogEntry(`TRAUMA DRAWN: ${cardDef.name}! ${cardDef.description}`, "trauma_major");
                         // Specific logic for Disorientation is handled in main.js's card play handler for now
                    }
                } else {
                     UIManager.addLogEntry(`Drew an unknown concept (ID: ${cardId}).`, "error");
                }
            }
        }
        return drawnCards;
    ...
    playCardFromHand(cardId) {
        ...
        if (cardIndex > -1) {
            ...
            if (!cardDef) {
                ...
                UIManager.addLogEntry(`Error: Unknown concept ID "${cardId}" in hand.`, "error");
                return null;
            }
            // Focus check is now primarily in EncounterManager or Game loop before calling this
            ...
            //     return null;
            // }
            // Actual focus deduction should happen in EncounterManager or Game loop just before effect.

            const playedCardId = this.hand.splice(cardIndex, 1)[0];
            if (cardDef.type === "Trauma") {
                this.discardPile.push(playedCardId); // Traumas go to discard after playing (if they have a play effect)
            } else {
                this.discardPile.push(playedCardId);
            }
            return cardDef; // Return card definition for effect processing
        }
        console.warn("Card not found in hand for playCardFromHand:", cardId, this.hand);
        return null;
    }

    discardCardFromHandById(cardId) {
        const cardIndex = this.hand.indexOf(cardId);
        if (cardIndex > -1) {
    ...
    discardRandomCardFromHand() {
        if (this.hand.length > 0) {
            ...
            const cardIdToDiscard = this.hand[randomIndex];
            return this.discardCardFromHandById(cardIdToDiscard);
        }
        return null;
    }


    addConceptToDeck(cardId, shuffleIn = false) {
        this.deck.push(cardId);
        if (shuffleIn) this.shuffleArray(this.deck);
    ...
    modifyFocus(amount, source = "internal reserves") { const prevFocus = this.focus; this.focus += amount; if (this.focus > this.maxFocus) this.focus = this.maxFocus; if (this.focus < 0) this.focus = 0; if (amount < 0 && this.focus < prevFocus) { UIManager.addLogEntry(`Focus expended by ${source}.`, "system_subtle");}}
    ...
    modifyClarity(amount, source = "the journey") { const prevClarity = this.clarity; this.clarity += amount; if (this.clarity > this.maxClarity) this.clarity = this.maxClarity; if (this.clarity < 0) this.clarity = 0; if (amount < 0) UIManager.addLogEntry(`The way forward blurs (-${-amount} Clarity from ${source}).`, "system_negative"); else if (amount > 0 && this.clarity > prevClarity) UIManager.addLogEntry(`A moment of lucidity (+${amount} Clarity from ${source}).`, "system_positive"); if (this.clarity === 0 && prevClarity > 0) { UIManager.addLogEntry("Clarity exhausted! A thick Mental Fog descends, oppressive and disorienting.", "critical"); this.modifyDespair(2, "Mental Fog"); /* Game.triggerMentalFogEvent(); */ }}
    modifyHope(amount, source = "an unknown influence") { const prevHope = this.hope; this.hope += amount; if (this.hope > this.maxHope) this.hope = this.maxHope; if (this.hope < 0) this.hope = 0; if (amount < 0) UIManager.addLogEntry(`A flicker of Hope gutters and fades (-${-amount} from ${source}).`, "system_negative"); else if (amount > 0 && this.hope > prevHope) UIManager.addLogEntry(`A fragile Hope glimmers anew (+${amount} from ${source}).`, "system_positive_strong"); if (this.hope <= 1 && prevHope > 1 && this.memories.some(mem => mem.id === "MEM_TARNISHED_LOCKET")) { UIManager.addLogEntry("The Tarnished Locket emits a faint, comforting warmth.", "artifact_effect"); this.tarnishedLocketHopeAppliedThisTurn = false; /* Reset for next turn check */ }}
    modifyDespair(amount, source = "the encroaching void") { const prevDespair = this.despair; this.despair += amount; if (this.despair > this.maxDespair) this.despair = this.maxDespair; if (this.despair < 0) this.despair = 0; if (amount > 0 && this.despair > prevDespair) UIManager.addLogEntry(`A cold Despair seeps in (+${amount} from ${source}).`, "system_negative"); else if (amount < 0) UIManager.addLogEntry(`The weight of Despair lessens slightly (-${-amount} from ${source}).`, "system_positive"); if (this.despair >= this.maxDespair && prevDespair < this.maxDespair) { UIManager.addLogEntry("Despair has reached critical levels! The shadows writhe with unseen horrors.", "critical_system"); /* Game.triggerCriticalDespairEvent(); */ } else if (this.despair > this.maxDespair * 0.7 && prevDespair <= this.maxDespair * 0.7) { UIManager.addLogEntry("Despair grows heavy. The Inner Sea feels more oppressive.", "warning"); }}
    ...
    modifyAttunement(attunementKey, amount, source = "an experience") { if (this.attunements.hasOwnProperty(attunementKey)) { const prevValue = this.attunements[attunementKey]; this.attunements[attunementKey] += amount; if (this.attunements[attunementKey] < 0) this.attunements[attunementKey] = 0; /* Add max caps if (this.attunements[attunementKey] > 10) this.attunements[attunementKey] = 10; */ if (this.attunements[attunementKey] !== prevValue) { UIManager.addLogEntry(`Your approach to ${ATTUNEMENT_DEFINITIONS[attunementKey].name} shifts via ${source} (${amount > 0 ? '+' : ''}${amount}). Current: ${this.attunements[attunementKey]}.`, "system"); } } }

    // --- Memories (Artifacts) ---
    addMemory(memoryId) { /* ... (Identical to previous Player.js) ... */ const memoryDef = MEMORY_DEFINITIONS[memoryId]; if (memoryDef && !this.memories.some(mem => mem.id === memoryId)) { this.memories.push({ ...memoryDef }); UIManager.addLogEntry(`A forgotten Memory surfaces: "${memoryDef.name}".`, "reward"); } else if (this.memories.some(mem => mem.id === memoryId)) { UIManager.addLogEntry(`You already possess the Memory: "${memoryDef.name}".`, "system"); } else { UIManager.addLogEntry(`Failed to acquire unknown Memory: ${memoryId}.`, "error"); } }
    removeMemory(memoryId) { /* ... (Identical to previous Player.js) ... */ const memoryIndex = this.memories.findIndex(mem => mem.id === memoryId); if (memoryIndex > -1) { const removedMemory = this.memories.splice(memoryIndex, 1)[0]; UIManager.addLogEntry(`The Memory of "${removedMemory.name}" fades...`, "system_negative"); return true; } return false; }
    hasMemory(memoryId) { return this.memories.some(mem => mem.id === memoryId); }

    // --- Persona Stance ---
    setPersonaStance(stanceId) { /* ... (Identical to previous Player.js, including attunementReq check) ... */ if (stanceId === null) { if (this.activePersonaStance) { UIManager.addLogEntry(`You release the ${this.activePersonaStance.name}, returning to a neutral mindset.`, "system"); } this.activePersonaStance = null; return; } const stanceDef = PERSONA_STANCE_DEFINITIONS[stanceId]; if (stanceDef) { let canAdopt = true; if (stanceDef.attunementReq) { for (const attKey in stanceDef.attunementReq) { if (this.attunements[attKey] < stanceDef.attunementReq[attKey]) { canAdopt = false; UIManager.addLogEntry(`Cannot adopt ${stanceDef.name}: requires ${ATTUNEMENT_DEFINITIONS[attKey].name} ${stanceDef.attunementReq[attKey]}. You have ${this.attunements[attKey]}.`, "warning"); break; } } } if (canAdopt) { this.activePersonaStance = { ...stanceDef }; UIManager.addLogEntry(`You adopt the ${this.activePersonaStance.name}.`, "system_positive"); } } else { UIManager.addLogEntry(`Unknown Persona Stance ID: ${stanceId}`, "error"); } }

    // --- Utility / Data Getters ---
    getUIData() { /* ... (Identical) ... */ return { integrity: this.integrity, maxIntegrity: this.maxIntegrity, focus: this.focus, maxFocus: this.maxFocus, clarity: this.clarity, maxClarity: this.maxClarity, hope: this.hope, maxHope: this.maxHope, despair: this.despair, maxDespair: this.maxDespair, attunements: this.attunements }; }
    getHandCardDefinitions() { return this.hand.map(cardId => CONCEPT_CARD_DEFINITIONS[cardId]).filter(Boolean); }
    getFullDeckCardDefinitions() { const allCardIds = [...this.deck, ...this.hand, ...this.discardPile]; return allCardIds.map(cardId => CONCEPT_CARD_DEFINITIONS[cardId]).filter(Boolean); }
    getTraumaCountInPlay() { /* ... (Identical) ... */ let count = 0; const pilesToScan = [this.deck, this.hand, this.discardPile]; pilesToScan.forEach(pile => { pile.forEach(cardId => { const cardDef = CONCEPT_CARD_DEFINITIONS[cardId]; if (cardDef && cardDef.type === "Trauma") { count++; } }); }); return count; }
    ...
    resetForNewRun() {
        ...
        this.despair = PLAYER_INITIAL_STATS.despair; this.insight = PLAYER_INITIAL_STATS.insight; // Reset run insight
        this.attunements = { ...PLAYER_INITIAL_STATS.attunements };
    ...
    prepareForEncounter() {
        this.discardPile.push(...this.hand.filter(id => id !== "AWK001")); // Keep Grasp if by some miracle it's still there and not played
        this.hand = this.hand.filter(id => id === "AWK001"); // Only keep Grasp if it wasn't played. Should be empty usually.
        // Actual encounter draw is now handled by startTurnInEncounter called by EncounterManager
        if (!this.activePersonaStance) { // Default to Observer if no stance chosen
            this.setPersonaStance("OBSERVER");
        }
    }

    startTurnInEncounter() { // Called by EncounterManager
        this.tarnishedLocketHopeAppliedThisTurn = false; // Reset locket flag
        let focusToRegen = this.maxFocus; // Base: full regen
        ...
        // Tarnished Locket effect check at start of player's turn IN ENCOUNTER
