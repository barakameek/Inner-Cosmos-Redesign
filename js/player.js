...
class Player {
            ...
            if (cardId) {
                ...
                if (cardDef) {
                    ...
                    if (cardDef.type === "Trauma" && cardDef.onDrawFunctionName) {
                         ...
                         // Actual onDraw effect is orchestrated by Game/EncounterManager after this draw.
                    }
    ...
    playCardFromHand(cardId) {
        ...
        if (cardIndex > -1) {
            ...
            // Focus check should be done by the calling context (EncounterManager or Game for out-of-combat)
    ...
    modifyFocus(amount, source = "internal reserves") { const prevFocus = this.focus; this.focus += amount; if (this.focus > this.maxFocus) this.focus = this.maxFocus; if (this.focus < 0) this.focus = 0; if (amount < 0 && this.focus < prevFocus && source !== "start of turn") { UIManager.addLogEntry(`Focus expended by ${source}.`, "system_subtle");}} // Quieter logging for focus unless it's a gain or specific named spend
    ...
    modifyClarity(amount, source = "the journey") { const prevClarity = this.clarity; this.clarity += amount; if (this.clarity > this.maxClarity) this.clarity = this.maxClarity; if (this.clarity < 0) this.clarity = 0; if (amount < 0) UIManager.addLogEntry(`The way forward blurs (-${-amount} Clarity from ${source}).`, "system_negative"); else if (amount > 0 && this.clarity > prevClarity) UIManager.addLogEntry(`A moment of lucidity (+${amount} Clarity from ${source}).`, "system_positive"); if (this.clarity === 0 && prevClarity > 0) { UIManager.addLogEntry("Clarity exhausted! A thick Mental Fog descends, oppressive and disorienting.", "critical"); this.modifyDespair(2, "Mental Fog"); Game.triggerMentalFogEffects(); }}
    ...
    modifyDespair(amount, source = "the encroaching void") { const prevDespair = this.despair; this.despair += amount; if (this.despair > this.maxDespair) this.despair = this.maxDespair; if (this.despair < 0) this.despair = 0; if (amount > 0 && this.despair > prevDespair) UIManager.addLogEntry(`A cold Despair seeps in (+${amount} from ${source}).`, "system_negative"); else if (amount < 0) UIManager.addLogEntry(`The weight of Despair lessens slightly (-${-amount} from ${source}).`, "system_positive"); if (this.despair >= this.maxDespair && prevDespair < this.maxDespair) { UIManager.addLogEntry("Despair has reached critical levels! The shadows writhe with unseen horrors.", "critical_system"); Game.triggerCriticalDespairEffects(); } else if (this.despair > this.maxDespair * 0.7 && prevDespair <= this.maxDespair * 0.7) { UIManager.addLogEntry("Despair grows heavy. The Inner Sea feels more oppressive.", "warning"); }}
    ...
    getTraumaCountInPlay() { let count = 0; const pilesToScan = [this.deck, this.hand, this.discardPile]; pilesToScan.forEach(pile => { pile.forEach(cardId => { const cardDef = CONCEPT_CARD_DEFINITIONS[cardId]; if (cardDef && cardDef.type === "Trauma") { count++; } }); }); return count; }
    ...
    prepareForEncounter() {
        this.discardPile.push(...this.hand); // Move entire hand to discard before encounter draw
        this.hand = [];
        if (!this.activePersonaStance) { this.setPersonaStance("OBSERVER"); }
        // Encounter specific draw is handled by startTurnInEncounter
    }
