...
const Game = (() => {
    ...
    function init() {
        console.log("Sunless Psyche Main (v2 Awakening - Debug 2): Initializing...");
        isGameOver = false;
        ...
        console.log("Sunless Psyche Main (v2 Awakening - Debug 2): Initialization complete.");
    }
    ...
    function _switchToView(viewId) { /* ... (Identical) ... */ console.log(`Main: Switching view to: ${viewId}`); currentViewId = viewId; uiMgr.showView(viewId); if (viewId === 'map-view') { _updateAndRenderNodeMap(); } }
    ...
    function startEncounterFromQueue(aspectIdToStart, previousViewBeforeStorylet) { /* ... (Identical) ... */ if (isGameOver || !aspectIdToStart) return; if (encounterMgr.startEncounter(aspectIdToStart, previousViewBeforeStorylet)) {} else { uiMgr.addLogEntry(`Failed to start queued encounter: ${aspectIdToStart}`, "error"); storyletEnded(); } }
    function _handleEncounterCardPlay(cardId) { /* ... (Identical, uses confirm()) ... */ if (isGameOver || !encounterMgr.isActive()) return; const cardDef = CONCEPT_CARD_DEFINITIONS[cardId]; if (!cardDef) return; if (cardId !== "TRM001" && encounterMgr.canSpendClarityForDisorientation && encounterMgr.canSpendClarityForDisorientation()) { if (confirm("Disorientation increases Concept costs by 1 Focus this turn.\nSpend 1 Clarity to negate this effect?")) { if(encounterMgr.spendClarityForDisorientation) encounterMgr.spendClarityForDisorientation(); } } encounterMgr.playConceptCard(cardId); refreshPlayerUI(); _checkGameOver(); }
    ...
    function refreshPlayerUI() { /* ... (Identical) ... */ uiMgr.updatePlayerStats(currentPlayer.getUIData()); uiMgr.updatePlayerHand(currentPlayer.getHandCardDefinitions()); uiMgr.updateDeckInfo(currentPlayer.deck.length, currentPlayer.hand.length, currentPlayer.discardPile.length, currentPlayer.getTraumaCountInPlay()); uiMgr.updateActiveMemories(currentPlayer.memories); }
    ...
    function _setupGlobalEventListeners() {
        ...
        if (storyChoicesContainer) {
            storyChoicesContainer.addEventListener('click', (event) => { 
                // *** ADDED CHECK FOR ACTIVE VIEW AND PREVENT DOUBLE PROCESSING ***
                if (currentViewId !== 'storylet-view' || uiMgr.getDOMElement('storyletView').classList.contains('view-hidden')) {
                    // If storylet view isn't active, or somehow the event fires when it's hidden, ignore.
                    // This can happen if the view is rapidly switched and an old event bubbles.
                    console.warn("Storylet choice click ignored: Storylet view not active.");
                    return; 
                }

                if (event.target.tagName === 'BUTTON' && event.target.dataset.choiceIndex && !encounterMgr.isActive()) {
                    const button = event.target;
                    if(button.disabled) return; // Prevent clicking already disabled button

                    button.disabled = true; // Temporarily disable to prevent rapid double clicks
                    
                    _handleStoryletChoice(parseInt(event.target.dataset.choiceIndex)); 
                    
                    // Re-enable button logic would be tied to when new choices are presented
                    // or if the same storylet re-displays with different conditions.
                    // For now, new storylet display will create new buttons.
                }
            });
        }
